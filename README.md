# 👋 Hi, I'm [GTRe5]

🎯 Data Analyst | Machine Learning Engineer | Deep Learning Enthusiast  
📍 Based in Vietnam  
💡 Passionate about Data, AI, and building intelligent systems  

---

## 🧠 About Me
- 🎓 Studying at Ton Duc Thang University
- 📊 Strong in Data Analysis, Machine Learning, Deep Learning
- 🔍 Interested in: NLP, Computer Vision, Recommendation Systems
- 🚀 Goal: Become AI Engineer / Data Scientist
- 📫 Email: hungpro123b@gmail.com

---

## ⚡ Tech Stack

### 🧑‍💻 Languages
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-003B57?style=for-the-badge&logo=database&logoColor=white)

### 📊 Data & ML
![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas)
![NumPy](https://img.shields.io/badge/Numpy-013243?style=for-the-badge&logo=numpy)
![Scikit-learn](https://img.shields.io/badge/ScikitLearn-F7931E?style=for-the-badge&logo=scikit-learn)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=for-the-badge&logo=pytorch)

### 🛠 Tools
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux)

---

## 📂 Featured Projects

### 🔹 [AI Image Captioning (COCO Dataset)](https://github.com/GTRe5/project1)
📌 Deep learning model combining CNN + LSTM + Attention  
⚙️ Tech: PyTorch, ResNet, NLP  
📊 Achievements:
- BLEU score improvement with Attention mechanism
- Compared multiple architectures (baseline vs full model)

---

### 🔹 [Market Basket Analysis](https://github.com/GTRe5/project2)
📌 Association Rule Mining with FP-Growth / Apriori  
⚙️ Tech: PySpark, Hadoop  
📊 Achievements:
- Found strong rules (support/confidence/lift)
- Optimized performance with distributed computing

---

### 🔹 [Data Analysis Dashboard](https://github.com/GTRe5/project3)
📌 Data visualization & insights dashboard  
⚙️ Tech: Python, Pandas, Power BI / Streamlit  
📊 Achievements:
- Cleaned & transformed large dataset
- Generated actionable insights

---

## 📊 GitHub Analytics

### 🔥 Stats
![GitHub stats](https://github-readme-stats.vercel.app/api?username=GTRe5&show_icons=true&theme=tokyonight)

### 📈 Most Used Languages
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=GTRe5&layout=compact&theme=tokyonight)

### 🧮 Contribution Graph
![GitHub Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=GTRe5&theme=tokyo-night)

---

## 🏆 Achievements

- 🥇 Built end-to-end ML/DL projects (Data → Model → Evaluation)
- 📊 Experience with real-world datasets (COCO, DAQUAR, Market Basket)
- ⚡ Optimized big data processing with PySpark
- 🤖 Implemented Attention mechanisms in deep learning models

---

## 🌐 Connect with Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/pqh2005)  
[![Portfolio](https://img.shields.io/badge/Portfolio-black?style=for-the-badge&logo=vercel)]([https://yourwebsite.com](https://portfolio-gtre5.vercel.app/))

---

## 👀 Profile Views
![Visitor Count](https://komarev.com/ghpvc/?username=GTRe5)

---

⭐️ From [GTRe5] — Building AI for the future 🚀
